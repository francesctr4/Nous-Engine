...
{
    const std::string result = nous::engine::filesystem::NormalizePath("Assets\\Shaders\\test.glsl");
    EXPECT_EQ(result, "Assets/Shaders/test.glsl");
}

TEST(t_FileSystem, NormalizePath_ForwardSlashPath_Unchanged)
{
    const std::string result = nous::engine::filesystem::NormalizePath("Assets/Shaders/test.glsl");
    EXPECT_EQ(result, "Assets/Shaders/test.glsl");
}
...
TEST(t_FileSystem, GetFilename_ReturnsBaseNameWithoutExtension)
{
    ...
    EXPECT_EQ(nous::engine::filesystem::GetFilename("Assets/Shaders/test.glsl"), "test");
}

TEST(t_FileSystem, GetFilename_FileWithMultipleDots)
{
    EXPECT_EQ(nous::engine::filesystem::GetFilename("BuiltIn.MaterialShader.vert.glsl"), "BuiltIn.MaterialShader.vert");
}

TEST(t_FileSystem, GetExtension_ReturnsExtensionWithDot)
{
    EXPECT_EQ(nous::engine::filesystem::GetExtension("test.glsl"), ".glsl");
}

TEST(t_FileSystem, GetExtension_NoExtension_ReturnsEmpty)
{
    EXPECT_EQ(nous::engine::filesystem::GetExtension("noextension"), "");
}
...
TEST(t_FileSystem, GetExtension_DotFile_ReturnsFull)
{
    ...
    EXPECT_EQ(nous::engine::filesystem::GetExtension("file.cpp"), ".cpp");
}
...
{
    EXPECT_TRUE(nous::engine::filesystem::Exists(m_tempDir));
}

TEST_F(t_FileSystemOps, Exists_ReturnsFalseForNonExistentPath)
{
    EXPECT_FALSE(nous::engine::filesystem::Exists(m_tempDir + "/does_not_exist"));
}

TEST_F(t_FileSystemOps, IsDirectory_TrueForDirectory)
{
    EXPECT_TRUE(nous::engine::filesystem::IsDirectory(m_tempDir));
}
...
TEST_F(t_FileSystemOps, DeleteDirectory_RemovesDirectory)
{
    ...
    EXPECT_TRUE(nous::engine::filesystem::DeleteDirectory(subDir));
...
TEST_F(t_FileSystemOps, DeleteFile_RemovesFile)
{
    ...
    EXPECT_TRUE(nous::engine::filesystem::DeleteFile(filePath));
    EXPECT_FALSE(fs::exists(filePath));
}

TEST_F(t_FileSystemOps, DeleteFile_NonExistentFile_ReturnsFalse)
{
    EXPECT_FALSE(nous::engine::filesystem::DeleteFile(m_tempDir + "/ghost.txt"));
}
...
TEST_F(t_FileSystemOps, CopyFile_CreatesCopyAtDestination)
{
    ...
    EXPECT_TRUE(nous::engine::filesystem::CopyFile(src, dst));
