...
public:
	...
	NOUS_ENGINE_API ResourceBase(uint32_t uID, ResourceType type);
	NOUS_ENGINE_API virtual ~ResourceBase();

	NOUS_ENGINE_API void SetName(std::string_view name);
	NOUS_ENGINE_API void SetUID(uint32_t uid);
	...
	[[nodiscard]] NOUS_ENGINE_API uint32_t GetUID() const;
	...
	[[nodiscard]] NOUS_ENGINE_API uint32_t GetReferenceCount() const;
...
private:
	...
	// it. A plain uint32_t lost increments under concurrent load — an undercount
