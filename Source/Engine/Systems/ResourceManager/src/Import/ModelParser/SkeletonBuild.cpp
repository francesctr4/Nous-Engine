...
namespace nous::engine::resource_manager
{
                               ...
                               const std::unordered_set<std::string>& animatedNames)
    {
        ...
        // Then propagate DOWN to descendants. BuildSkeleton already keeps ancestors, so
        // between the two the skeleton becomes the whole subtree SPANNED by the animated
        // nodes. Without this every joint a clip does not drive is pruned: Mixamo animates
        // 52 of its 65 joints, the other 13 being "_End" leaf terminators, so the result
        // would be a 53-bone rig that disagrees with its own skinned sibling.
        //
