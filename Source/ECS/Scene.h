...
public:
    ...
    void Serialize(const std::string& filepath) const {
        ...
        printf("Scene saved: %s with %zu objects\n", filepath.c_str(), m_GameObjects.size());
    }
    ...
    void Deserialize(const std::string& filepath) {
        ...
        if (!arr) {
            json_value_free(root);
            printf("No GameObjects array found in scene file\n");
            return;
        }
        ...
        for (auto& gameObject : m_GameObjects) {
            ...
            if (parentID != 0) {
                ...
                if (parent) {
                    ...
                    printf("Set parent: %s (ID: %u) -> %s (ID: %u)\n",
                           gameObject->GetName().c_str(), gameObject->GetID(),
                           parent->GetName().c_str(), parentID);
                } else {
                    printf("Warning: Parent with ID %u not found for GameObject %s. It will be a root object.\n",
        ...
        json_value_free(root);
        printf("Successfully loaded scene: %s with %zu objects\n",
               filepath.c_str(), m_GameObjects.size());
    }
