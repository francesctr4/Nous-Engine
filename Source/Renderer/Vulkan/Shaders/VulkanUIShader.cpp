#include "Renderer/Vulkan/Shaders/VulkanUIShader.h"
...
void NOUS_VulkanUIShader::UIShaderSetModel(VulkanContext* vkContext, VulkanUIShader* shader, glm::mat4x4 model)
{
    if (vkContext && shader)
    {
        uint32 imageIndex = vkContext->imageIndex;
        ...
        vkCmdPushConstants(command_buffer, shader->pipeline.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4x4), &model);
    }
}
